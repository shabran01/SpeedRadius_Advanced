...
### FIXED: System Info Gauge Template Compilation

**`system/plugin/ui/system_info.tpl`**

- Removed JavaScript object literals that Smarty interpreted as template syntax.
- The System Info page now compiles correctly while keeping remote gauge polling enabled.

## [Unreleased] - 2026-09-08

---

### ADDED: Remote Server Gauges to System Info

**`system/plugin/system_info.php` + `system/plugin/ui/system_info.tpl` + `config.sample.php`**

- Added admin-authenticated CPU, memory, swap, disk, uptime, load, and hostname gauges to `?_route=plugin/system_info`.
- Gauges read the remote VPS over SSH and refresh every five seconds.
- Added optional `SSH_HOST`, `SSH_PORT`, `SSH_USER`, and `SSH_PASS` configuration settings.
- Missing SSH configuration is reported in the page without breaking the existing System Info page.
