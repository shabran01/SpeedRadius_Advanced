...
### 🐛 FIXED: Customer View ON/OFF Toggle — Now Shows Real Router State

**`system/controllers/customers.php` · `system/helpers/mikrotik_device_info.php` · `ui/ui/customers-view.tpl`**

The ON/OFF toggle button was incorrectly showing the customer's database status, which doesn't reflect the actual MikroTik router enable/disable state.

- **New helper** `get_customer_router_state()` queries the MikroTik router directly (`/ip hotspot user print` / `/ppp secret print`) to read the `disabled` property and returns `'on'`, `'off'`, or `'unknown'`.
- The controller now calls this helper and assigns `$customer_router_state` to the template.
- The template uses the **real router state**:
  - **ON** (green) → customer is truly enabled on the router
  - **OFF** (grey) → customer is truly disabled on the router
  - **Unknown** → falls back to traditional Enable/Disable buttons (when router unreachable)
...
### ✨ NEW: Auto-Detecting Payment Progress Page (replaces blocking alert)

**`system/plugin/lib_payment_progress.php` (new) · `system/plugin/payment_status.php` (new) · `system/plugin/initiatetillstk.php` · `system/plugin/initiatebankstk.php` · `system/plugin/initiatempesa.php` · `system/plugin/initiatepayhero.php`**

After an M-Pesa STK push, the old blocking `alert()` ("...enter your PIN...") is replaced with a **live payment-progress page** that:

- Shows a **spinning countdown ring + "Time remaining: 35s"** timer.
- **Polls the server every 3 seconds** via a new JSON endpoint (`plugin/payment_status/{id}`) to auto-detect whether the payment was confirmed.
- **Auto-redirects** to the order view the moment M-Pesa confirms the payment (green "✓ Payment received!").
- Shows a clear **failed/cancelled** message if the transaction errors, and a "not completed" state after ~35s.

Applied to all STK initiators: **MpesatillStk, BankStkPush, mpesa, and PayHero** (via one shared renderer — no code duplication).
...
### 🚀 IMPROVED: Traffic Monitor — Much Faster & Accurate Real-Time from MikroTik

**`system/plugin/traffic_monitor.php` + `system/plugin/ui/traffic_monitor.tpl`**

The Live Traffic Monitor page was slow and often stuck at 0 bps because it opened a **new MikroTik API connection every second** (no connection timeout) and blocked page-load on `/interface/print`. Fixed for speed + accuracy:

- **Connection timeout capped at 5s** — no more long hangs when a router is slow/unreachable.
- **Server-side traffic cache** (new `tbl_traffic_monitor` table, auto-created) — samples cached for ~1.6s so all open tabs **share one router poll** instead of each opening its own connection (connection storm fixed → faster, more accurate).
- **Interface list cached for 60s** — page loads instantly without blocking on the router.
- **Graceful fallback** — returns last known values when the router is unreachable (instead of dropping to 0).
- **Live status badge** — shows "LIVE • last: hh:mm:ss" while data flows, and turns red "OFFLINE" when the router can't be reached (no more silent zeros).

### 🎨 UI: Compact Medium Sizing

- Page title, LIVE badge, router tabs, gauge cards, speed numbers, progress bars and the chart card were all reduced to a **clean medium size** (not oversized), keeping the light modern theme.
...
### ✨ REDESIGNED: Live Traffic Monitor UI — Clean Light Theme

**`system/plugin/ui/traffic_monitor.tpl`**

The Live Traffic Monitor page (`/?_route=plugin/traffic_monitor_ui`) was redesigned into a **clean, modern, light theme** (replacing the earlier dark theme) that is fully responsive on all device types:

- **Light gradient background** (soft slate → indigo → violet tint) with white cards, subtle borders and soft shadows.
- **Ring gauges** for Download (RX) and Upload (TX) on light backgrounds with sky-blue / violet gradient rings.
- **Clean pill router tabs** with an indigo→violet active gradient.
- **Light-themed Chart.js** chart with soft grid lines, dark text and matching gradient fills.
- **Fully responsive**: single-column on mobile (cards stack, gauges centered), two-column on tablet/desktop; interface dropdown and peak stat chips wrap cleanly; chart scales via a fixed-height wrapper (`maintainAspectRatio:false`).
...
### 🐛 FIXED: Smarty Template Compilation Error on Router Notifier Page

**`system/plugin/ui/router_status_notifier.tpl`**

The redesigned template threw a Smarty syntax error on the server because CSS curly braces (`{` `}`) were being parsed as Smarty tags.

- Wrapped the entire `<style>` block in `{literal}...{/literal}` so Smarty ignores the CSS.
- Wrapped the bottom DataTables JavaScript in `{literal}...{/literal}` (same convention used by other templates in the project).
- The page now renders correctly without server-side compilation errors.
