...
### CHANGED: "Pay For a TV" moved above the account login form

**`system/plugin/download.php`**

- The trigger card now sits directly under the package cards, **above** the "Already Have an Active Package?" form and the user-login form.
- Previously it sat below both login forms and the device-info card — so a customer whose TV cannot display the page had to scroll past two forms that do not apply to them before finding the one path built for their problem. The people who need it are exactly the people those forms cannot help.
- Only the trigger card moved. The modal itself was left where it was: it is `position:fixed`, so its position in the DOM has no effect on where it appears, and moving it would have been risk without benefit.
- The card keeps its place in the page flow, so nothing else shifted.

## [2.2.39] - 2026-09-22

---

### FIXED: A TV purchase overwrote the device's own account number

**`system/plugin/download.php`**

After paying for a TV, the phone's **"Enter your account number or user name"** box started showing the account that had just been created for the TV. On a phone that had bought nothing for itself, it looked as though that device owned the TV's package.

**Cause:** `submitTvPay()` called `persistAccountId()` — the helper that reads, creates and stores *this device's* account in the `accountId` cookie — and then wrote the account returned by the payment endpoint straight back into the same cookie. So a TV purchase replaced the device's own account reference with the TV's.

### FIXED: TV purchases now use their own cookie

- The TV flow reads and writes **`tvAccountId`**, a separate cookie, and never touches `accountId`.
- `persistAccountId()` is no longer called from the TV path at all, so the device's own account is left completely alone by a TV purchase.
- The retry path keeps using the same TV account, since `tvBindCtx` is built from the value returned by that flow.

> **Residual state to be aware of:** the incorrect cookie was already written to any device that completed a TV purchase before this release, and it is stored for **7 days**. So the box may keep showing the TV's account number until that cookie expires or is cleared — the fix stops it happening again, it does not retroactively undo it. Clearing the browser's cookies for the site resolves it immediately. Any "2 mins test" purchase made while testing is also long expired.
...
### FIXED: Devices bound through "Pay For a TV" ran at full line speed

**`system/plugin/download.php`, `system/cron.php`**

A package with a 4 Mbps limit was delivering full speed to bound devices.

**Why:** the speed limit lives on the hotspot **user profile** (`rate-limit`). A `bypassed` device never authenticates, so RouterOS never applies a profile to it and therefore never creates a queue — it routes straight through unshaped. Nothing was misconfigured; the limit simply had no path to reach that device. This is the visible half of the `bypassed` trade-off.

### FIXED: `tv_bind` now gives the device its own queue

- After writing the binding, `tv_bind` creates a `/queue simple` entry for the device: `target=<ip>/32`, `max-limit=<rate>`, name `SR-tv-<mac>`, carrying the same `SR|…` comment as the binding.
- **The rate is copied off the hotspot profile**, not rebuilt from the plan. That guarantees the device receives exactly the speed a normal login would have got, and the two cannot drift apart when a plan is edited. It falls back to `tbl_plans` → `tbl_bandwidth` only if the profile has no `rate-limit`.
- The profile value is trimmed to its first whitespace-separated token, because a profile `rate-limit` may carry a **burst** spec (e.g. `2M/4M 4M/8M 8/8 8/8`) that a simple queue's `max-limit` would reject.
- Idempotent: an existing `SR-tv-<mac>` queue is removed before re-adding, so re-purchasing refreshes the limit instead of stacking queues.
- **Shaping never fails the bind.** If the queue cannot be applied the device is still online, so the response carries `shape_warning` and the form shows it in amber. Encountering a failure on connectivity work should not cost the customer their purchase.
- Guards against an address ending in `.0` or `.255`: a queue aimed at a network or broadcast address shapes nothing, and one of those silently broke an earlier test binding. It warns instead of writing something meaningless.

### FIXED: The queue is removed when the package lapses

- `cron_cleanup_tv_bindings()` now removes the `SR-tv-<mac>` queue alongside the binding. Without this the limit would **outlive the package** and, worse, would go on limiting whatever device picked up that IP next.

> **Still not enforced for `bypassed` devices: the data cap.** A simple queue shapes bandwidth; it cannot enforce a package's time or volume quota. Time is handled (the cron removes the binding at expiry). A `limit-bytes-total` cap is **not** enforced on a bypassed device. Closing that would mean the cron checking the device's byte counters and either zeroing its `max-limit` or converting the binding to `blocked` once the cap is hit.
...
### FIXED: "Pay For a TV" took the payment but left the device offline

**`system/plugin/download.php`**

A test with a phone confirmed the money was taken and the binding was written, yet the device still had no internet. The binding was fine — the device was never **authenticated**.

**Root cause**

With `login-by=mac`, RouterOS does not log a device in as "whoever owns that MAC". It looks for a **hotspot user whose name *is* the MAC address**. The purchased account is named after the account id, so the MAC could never match it, and the hotspot left the device unauthenticated while every visible artefact (payment, recharge row, ip-binding) looked correct.

### FIXED: The device now gets its own hotspot user

- `tv_bind` now creates (or updates) a hotspot user **named after the MAC address**, carrying the same `profile` as the purchased account and mirrored `limit-uptime` / `limit-bytes-total` values read off that account.
- Reading the limits off the existing user rather than recomputing them from the plan means the device gets **exactly** the limits a normal login would have received, and it keeps working if the plan's limit logic changes later.
- The profile name is read from the purchased user first, falling back to `tbl_user_recharges.namebp`.
- The user is explicitly enabled after creation, matching what `MikrotikHotspot::addHotspotUser()` already does for RouterOS 7.18+.
- Still idempotent: an existing MAC-named user is updated rather than duplicated, so retrying is safe.

### NEW: Capability check runs BEFORE payment

- New `tv_check` endpoint plus a `tv_resolve_router()` / `tv_mac_login_enabled()` helper pair.
- Opening the TV form now asks the router whether any hotspot profile has `mac` in its `login-by` list. If it does not, the form explains that a TV cannot be connected yet and **disables the pay button**, stating plainly that the customer has not been charged.
- This exists specifically so we never again take money for a device we cannot connect. The previous flow could only discover the misconfiguration *after* payment.
- The bind response also returns `mac_login_enabled`, and the success message warns if it is false, so a silent misconfiguration stays visible during testing.

> **One-time router setting required for this to work:**
> ```
> /ip hotspot profile set [find] login-by=mac,http-chap,http-pap
> ```
> Adding `mac` to the existing `login-by` list is additive — existing username/password logins keep working. Until this is set, the form will correctly refuse to take payment.

## [2.2.33] - 2026-09-22

---

### NEW: "Pay For a TV" — buy a package for a device that cannot show the login page

**`system/plugin/download.php`**

Many smart TVs cannot render the hotspot login page at all, so the customer has no way to buy. The portal now carries a **Pay For a TV** button that opens a form where they enter the TV's MAC address on their phone, pick a package and pay by M-Pesa. Once the payment is confirmed the TV is bound on the router and connects without ever seeing the login page.

**The form**

- Device MAC Address (required), Device Name (optional), Select Package, Phone Number (M-Pesa).
- A collapsible **"How do I find my TV MAC address?"** panel with the menu path for Vitron, Samsung, LG, Sony/Android TV, Hisense and TCL/Roku, so the customer does not have to phone support.
- The MAC input formats itself as it is typed and accepts `AABBCCDDEEFF`, `AA-BB-CC-DD-EE-FF` or `AA:BB:CC:DD:EE:FF` — customers rarely type the colons.
- The package dropdown is populated from the **same** `plugin/hotspot_plan` call that builds the package cards, so the two can never offer different prices.

**Payment is reused, not rebuilt**

- The form calls the existing `plugin/CreateHotspotuser&type=grant` endpoint and waits on the existing `type=verify` poll, exactly like the normal Buy button. No second payment path was introduced.

**The binding**

- A new `tv_bind` POST handler resolves the router, looks the TV up in `/ip/dhcp-server/lease` by MAC and writes an `/ip/hotspot/ip-binding` entry.
- **IP lookup:** `/ip/hotspot/ip-binding` requires an `address`, not just a MAC — the project's own `ip_bindings_add()` rejects an entry without one. The address is read from the TV's DHCP lease, which exists as soon as the TV has joined the WiFi, even when it cannot display the login page. If there is no lease yet the customer is told to connect the TV, wait and retry.
- **Idempotent:** if the MAC already has a binding it is `set`, otherwise `add`ed, so retrying never stacks up duplicate entries.
- **Auditable:** the binding's `comment` records `SR|<username>|exp <expiry>|<device name>`, so bindings can be traced back to a customer and cleaned up after expiry.
- The binding type lives in a single constant, `TV_BINDING_TYPE`, defaulting to `regular`.

> **Router prerequisite most likely to bite:** with `type=regular`, the TV skips the login page *and* the plan's limits still apply — but only if the hotspot profile permits MAC login (`login-by` must include `mac`). If the profile does not, the binding will be created and the payment will succeed while the TV still has no internet. In that case change `TV_BINDING_TYPE` to `'bypassed'`, which is guaranteed to get the TV online but means the router will **not** enforce the plan's time or data limits.

### FIXED: Post-payment retry could have charged the customer twice

- The first draft reset the submit button to the payment step whenever the binding failed — so a customer whose TV had not yet appeared on the network would be sent back through a fresh M-Pesa STK push for a payment they had already made.
- Once payment is confirmed the button now becomes **Retry Connection** and re-runs *only* the binding. The bind endpoint independently verifies that an active recharge exists, so this is safe in both directions: an unpaid attempt is rejected, and a paid one is completed without any second charge.
- The payment-poll timeout takes the same path for the same reason — it can leave the payment state genuinely unknown, and retrying the binding resolves that safely instead of risking a duplicate charge.

## [2.2.32] - 2026-09-22

---

### CHANGED: Customer Profile Card Now Uses the Same Value Chips as the Package Card

**`ui/ui/customers-view.tpl`**

- The left-hand profile card was a wall of plain black right-aligned text with a single green pill on Status. It now uses the identical chip system introduced on the package card in 2.2.31, so the two cards read as one design:
  ...
  - **Username**, **Phone Number**, **Email**, **Account Type**, **PPPOE Username**, **PPPOE Remote IP** → slate monospace chips, marking them as identifiers you would copy out rather than prose
...
- **Status moved off `bg-green` / `bg-red` onto `sr-chip-ok` / `sr-chip-bad`.** Those two classes existed only for this one pill; using the shared chip classes means one colour definition drives both cards instead of two that could drift apart. The old rules are left in place since other markup may still use them.
...
- **Auto Renewal** displayed a bare lowercase `yes` / `no`; it now shows `Yes` / `No` via `Lang::T()`, consistent with how the package card renders Active.
...
### PERFORMANCE: Sync Opened a New RouterOS Connection for Every Single Customer

**`system/devices/MikrotikHotspot.php`, `system/devices/MikrotikPppoe.php`**

- `add_customer()` calls `getClient()` on every invocation, and `getClient()` had no memory of previous calls. Syncing 85 customers therefore performed **85 separate TCP connections, 85 logins, and 85 `/system/resource/print` probes** — the probe result is discarded, it exists purely to validate the connection.
- RouterOS API connections are reusable, so `getClient()` now keeps the client in a per-request pool keyed by `ip|username` and returns it on subsequent calls. For an 85-customer run on one router that is **85 connections down to 1**.
- Scope is strictly the request: each PHP request starts with an empty pool, and the cron/CLI paths run in their own process, so nothing is shared or leaked between requests.
- Added `dropClients()`, called from the failure path of `add_customer()` in both classes. Without it a dead socket would poison every remaining customer in the batch — previously each customer got a fresh connection, so the pool introduced that exposure and this closes it.

### PERFORMANCE: Every Customer Was Removed From the Router Three Times

**`system/devices/MikrotikHotspot.php`**

- `add_customer()` called `removeHotspotUser()`, then a conditional `removeHotspotActiveUser()`, and then `addHotspotUser()` — which *itself* begins by calling `removeHotspotUser()`. Since `removeHotspotUser()` starts by calling `removeHotspotActiveUser()`, that method ran **three times per customer**.
- `removeHotspotActiveUser()` is the most expensive call in the chain: per IP address it removes firewall connection-tracking entries and the DHCP lease, then does a full MAC-cookie print/remove pass. On a router with a busy connection table the conntrack query in particular is slow.
- The redundant removal in `add_customer()` is gone, along with the `$isExp` lookup that existed only to feed it. That single query now runs **once per customer instead of three times**, and one database lookup per customer was eliminated as a side effect.
- Behaviour is unchanged: `addHotspotUser()` still removes then re-adds the user, so counters and the active session are still reset exactly as before. Only the duplicated work was removed.

### CHANGED: Batch Pacing

**`ui/ui/plan-sync.tpl`**

- Inter-batch delay reduced from **500 ms to 150 ms**, and the per-batch AJAX timeout raised from **60 s to 90 s** so a slow batch is less likely to be abandoned mid-flight (an abandoned batch is retried at the same offset, which duplicates the work instead of saving it).

> **Expected effect:** per-customer RouterOS round trips drop from roughly 13–17 to about 8, on a connection that is established once per request instead of once per customer. The connection elimination is the dominant saving.
>
> **Measurement note:** if a run is still slow after this, the next thing to check is `tbl_logs` for `Connection attempt N failed` lines from `getClient()`. A first attempt failing triggers a **2 second `sleep()`** before the retry, and it is invisible in the UI because the retry usually succeeds — that shows up as several seconds of dead time per customer with zero reported errors. Also verify that a hotspot user profile exists on the router for every plan name in use; when a profile is missing, `addHotspotUser()` calls `add_plan()` for that customer.
...
### FIXED: Bulk Sync Reported "Success" for Customers It Never Synced

**`system/controllers/plan.php`, `system/devices/MikrotikHotspot.php`, `system/devices/MikrotikPppoe.php`**

- `plan/sync-process` discarded `add_customer()`'s return value and recorded `status: success, message: Synced` for every row regardless of the outcome. Both device classes signal failure by returning `false` — a router that cannot be resolved, a client that will not connect, or any caught exception all return `false` after logging to `_log`.
- The result was that a run against an unreachable router (or one with no active customers) displayed **Success: N, Errors: 0** while nothing at all reached the router. The on-screen counts were not evidence of anything.
- The return value is now checked with `=== false` rather than a truthy test, so device plugins that return nothing (`null`) are not misreported as failures. Failed rows now show *"Router unreachable, not found, or rejected the sync"* with their plan and router.
- Concrete case this now catches: rows whose `routers` is `balance` (written by the top-up flows in `Package.php`) or blank. `info()` returns `false` for those, and `MikrotikHotspot::add_customer()` previously dereferenced that `false` as an array — a PHP 8 warning swallowed into a `false` return. That method now has the same explicit `if (!$mikrotik)` guard `MikrotikPppoe` already had, so it returns a clean failure instead.

### FIXED: Sync Scope Filtered One Router but Wrote to Another

**`system/controllers/plan.php`, `system/devices/MikrotikHotspot.php`, `system/devices/MikrotikPppoe.php`**

- The page filtered customer rows on `tbl_user_recharges.routers`, then called `add_customer($customer, $plan)` — which resolves its router from **`tbl_plans.routers`**. The results log reported `$tur['routers']` in between, so the filter, the action and the report could all name different routers.
- These genuinely diverge: `customers/change_router_post` rewrites `tbl_user_recharges.routers` only, leaving the plan untouched. Selecting *Sync Scope → Router A* could therefore push customers onto Router B while the log said "Router: A".
- `add_customer()` now accepts an optional third argument holding the recharge row (or a plain router name) and targets that router when present. It stays backwards compatible for every existing 2-argument caller and for third-party device plugins, since the parameter is optional.
- This also repairs `customers/change_router_post`, which was already calling `add_customer($customer, $plan, $active_plan)` — a third argument that neither class accepted, so PHP silently discarded it and the sync-after-move went to the plan's router instead of the newly selected one.
- Uses `is_string()` / `is_array()` / `is_object()` checks before reading the third argument so a `false` from a failed lookup cannot trigger an "array offset on bool" warning.

### SECURITY: CSRF Token Now Required on `plan/sync-process`

**`system/controllers/plan.php`, `ui/ui/plan-sync.tpl`**

- `plan/sync-process` is a state-changing, destructive endpoint reachable by **GET** with no token. `_admin()` only verifies login and there is no global admin CSRF layer, so the only protection available is the per-action check that `customers/sync` already performs.
- An attacker-controlled page could therefore make a logged-in admin's browser walk the whole sync loop — which for hotspot means deleting and recreating every user.
- The endpoint now verifies `Csrf::check(_req('token'))`, and the page assigns `csrf_token` via `Csrf::generateAndStoreToken()` and sends it with both AJAX calls. `Csrf::check()` honours `$config['csrf_enabled']`, so sites with CSRF disabled are unaffected.
- Requests now return **HTTP 403** with `success: false` instead of `200` when rejected, and the JS treats a 403 as terminal: it stops the loop and reports *"Security token expired"* (tokens live 30 minutes) rather than retrying the same batch forever or showing a generic *"Connection error: error"*.
