...
class Message
{
    ...
    public static function sendWhatsapp($phone, $txt)
    {
        ...
        // First try using the WhatsApp Gateway plugin
        if (function_exists('whatsappGateway_hook_send_whatsapp')) {
            $result = whatsappGateway_hook_send_whatsapp([$phone, $txt]);
            if (!empty($result) && strpos($result, 'error') === false) {
                return true;
            }
        }

        // Fallback to default WhatsApp configuration
